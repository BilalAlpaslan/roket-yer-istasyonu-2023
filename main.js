  ...
  mainSerialPort.on("data", (data) => {
    ...
    while (buffer.indexOf("\n") !== -1) {
      if (counter == 0) {
        ...
        const temp = buffer.slice(0, buffer.indexOf("\n"));
        buffer = buffer.slice(buffer.indexOf("\n") + 1);
        datas.push(temp);
        saveLog("main.csv", temp + "\n");
        mainWindow.webContents.send("data-rec", temp);
      }
    }
  });
...
ipcMain.on("serial-send", (evetn, data) => {
  mainSerialPort.write(data.toString());
});
...
ipcMain.on("port-window-open", () => {
  createPortWindow();
});

ipcMain.on("port-window-close", () => {
  portWindow.close();
});
    ...
    payloadSerialPort.on("data", (data) => {
      ...
      if (buffer2.indexOf(finish_buffer) !== -1) {
        ...
        if (buffer2[30] == 0x0d && buffer2[31] == 0x0a) {
          ...
          payload_datas[0] = byteToFloat([
            buffer2[5],
            buffer2[6],
            buffer2[7],
            buffer2[8],
          ]).toFixed(4);
          payload_datas[1] = byteToFloat([
            buffer2[9],
            buffer2[10],
            buffer2[11],
            buffer2[12],
          ]).toFixed(4);
          payload_datas[2] = byteToFloat([
            buffer2[13],
            buffer2[14],
            buffer2[15],
            buffer2[16],
          ]).toFixed(4);
          payload_datas[3] = byteToFloat([
            buffer2[17],
            buffer2[18],
            buffer2[19],
            buffer2[20],
          ]).toFixed(4);
          payload_datas[4] = byteToFloat([
            buffer2[21],
            buffer2[22],
            buffer2[23],
            buffer2[24],
          ]).toFixed(4);
          payload_datas[5] = byteToFloat([
            buffer2[25],
            buffer2[26],
            buffer2[27],
            buffer2[28],
...
ipcMain.on("send-hyi", (event, data) => {
  if (HyiSerialPort != null) {
    ...
    } else console.log("Error: Port is not open");
  } else {
